...
function GymRooms({ onBack, isAdmin, user }) {
  ...
  const fetchSpaces = async () => {
    try {
      ...
      // Filter for gym rooms (case-insensitive check)
      const gymRooms = data.filter(space => 
        space.type && space.type.toLowerCase().includes('gym')
      )
      setSpaces(gymRooms)
    } catch (err) {
      setError(err.message)
      console.error('Error fetching gym rooms:', err)
    } finally {
      setLoading(false)
    }
  }
    ...
    return (
      <div className="gym-rooms-container">
        ...
        <div className="room-view-content">
          <div className="loading">Loading gym rooms...</div>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="gym-rooms-container">
      ...
      {spaces.length === 0 ? (
        <div className="room-view-content">
          <p className="no-rooms">No gym rooms found</p>
        </div>
      ) : (
